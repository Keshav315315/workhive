...
const StatCard = ({ label, value, icon: Icon, iconColor, iconBg, glowClass, accentHex, delay = 0 }) => (
  <motion.div
    ...
    whileHover={{ y: -4 }}
    className={`glass-card card-shine rounded-2xl p-5 cursor-default relative overflow-hidden card-hover ${glowClass}`}
  >
    {/* Bottom accent line */}
    <div className="absolute bottom-0 left-0 right-0 h-0.5 opacity-60" style={{ background: accentHex }} />
  ...
  return (
    ...
    >
      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
        ...
        task.status === 'review' ? 'bg-amber-400' : 'bg-slate-600'
      }`} />
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-medium truncate ${task.status === 'done' ? 'line-through text-slate-600' : 'text-white'}`}>
          {task.title}
        </div>
        <div className="text-xs text-slate-600 flex items-center gap-2 mt-0.5 flex-wrap">
          <span className="truncate">{task.project?.name}</span>
          {task.assignedTo && <span className="text-slate-700">→ {task.assignedTo.name}</span>}
          {task.dueDate && (
...
const ProjectCard = ({ project, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -3 }}
      className="card-shine glass-card rounded-2xl p-4 cursor-pointer relative overflow-hidden card-hover"
    >
      ...
      <Link to={`/projects/${project._id}`} className="block">
        <div className="flex items-start gap-3 mt-1">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${color}20`, border: `1px solid ${color}35` }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-slate-600 mt-0.5 flex items-center gap-1">
              ...
              <span className={`capitalize ${
                ...
                project.status === 'on-hold' ? 'text-amber-400' : 'text-indigo-400'
              }`}>{project.status}</span>
            </div>
          </div>
        </div>
...
const ViewAll = ({ to }) => (
  <Link to={to} className="text-xs text-indigo-400 hover:text-amber-400 flex items-center gap-1 transition-colors">
...
const EmptyState = ({ icon: Icon, text, sub, action }) => (
  <div className="text-center py-8">
    <Icon size={36} className="mx-auto mb-3 opacity-20 text-indigo-400" />
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <Shield size={20} className="text-indigo-400" /> Admin Dashboard
          </h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Team Members" value={stats.totalUsers || 0} icon={Users}
          iconColor="text-indigo-400" iconBg="bg-indigo-500/15"
          glowClass="stat-glow-indigo" accentHex="#6366f1" delay={0.05} />
        <StatCard label="Total Tasks" value={stats.total} icon={CheckSquare}
          iconColor="text-indigo-300" iconBg="bg-indigo-500/10"
          glowClass="stat-glow-indigo" accentHex="#6366f1" delay={0.1} />
        ...
        <StatCard label="Overdue" value={stats.overdue} icon={AlertTriangle}
          iconColor="text-red-400" iconBg="bg-red-500/15"
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <Panel>
          <PanelTitle icon={BarChart3} iconColor="text-indigo-400">Team Progress</PanelTitle>
          <div className="flex items-center gap-4 mb-3">
            <div className="flex-1">
              <p className="text-slate-600 text-xs mb-2">{stats.done} of {stats.total} tasks completed across all members</p>
      ...
      {stats.userStats?.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Panel>
            <PanelTitle icon={Users} iconColor="text-indigo-400">Member Performance</PanelTitle>
            <div className="space-y-3">
              {stats.userStats.map((u, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-white truncate mb-1">{u.name}</div>
                    <ProgressBar done={u.done} total={u.total} delay={0.1 + i * 0.05} />
                  </div>
                  <div className="flex gap-3 text-xs text-slate-500 flex-shrink-0 text-right">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }}>
          <Panel>
            <PanelTitle icon={CheckSquare} iconColor="text-indigo-400" action={<ViewAll to="/tasks" />}>
              Recent Tasks
            </PanelTitle>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
          <Panel>
            <PanelTitle icon={FolderKanban} iconColor="text-amber-400" action={<ViewAll to="/projects" />}>
              All Projects
            </PanelTitle>
            {projects.length === 0
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-600 text-sm mt-1">
            ...
            {stats.overdue > 0
              ? <span className="text-red-400">{stats.overdue} overdue task{stats.overdue !== 1 ? 's' : ''}</span>
              : 'All tasks on track'}
          </p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Completed" value={stats.done} icon={TrendingUp}
          ...
          glowClass="stat-glow-emerald" accentHex="#10b981" delay={0.15} />
        <StatCard label="Overdue" value={stats.overdue} icon={AlertTriangle}
          iconColor="text-red-400" iconBg="bg-red-500/15"
          glowClass="stat-glow-red" accentHex="#ef4444" delay={0.2} />
      </div>
        ...
        <Panel>
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="text-slate-600 text-xs mt-0.5">{stats.done} of {stats.total} tasks completed</div>
            </div>
          ...
          <Panel>
            ...
            {recentTasks.length === 0
              ? <EmptyState icon={CheckSquare} text="No tasks assigned to you yet." action={
                  <Link to="/tasks/new"><button className="mt-3 text-xs text-indigo-400 hover:text-amber-400 transition-colors">+ Add task</button></Link>
                } />
              : <div className="space-y-2">{recentTasks.map(t => <TaskItem key={t._id} task={t} />)}</div>
            }
          </Panel>
          ...
          <Panel>
            ...
            {projects.length === 0
              ? <EmptyState icon={FolderKanban} text="You're not in any projects yet." action={
                  <Link to="/projects"><button className="mt-3 text-xs text-indigo-400 hover:text-amber-400 transition-colors">+ New project</button></Link>
                } />
              : <div className="grid grid-cols-2 gap-3">{projects.map((p, i) => <ProjectCard key={p._id} project={p} index={i} />)}</div>
            }
          </Panel>
